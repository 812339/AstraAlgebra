...
int main() {
    std::cout << "=== AstraAlgebra Math Library Running Demo ===" << std::endl;
    
    // 1. 向量基本操作
    std::cout << "\n1. Vector Basic Operations:" << std::endl;
    ...
    // 5. 数学工具函数
    std::cout << "\n5. Math Utility Functions:" << std::endl;
    
    float radians = Math::degToRad(180.0f);
    float degrees = Math::radToDeg(Math::PI);
    float lerpValue = Math::lerp(0.0f, 10.0f, 0.5f);
    float clamped = Math::clamp(15.0f, 0.0f, 10.0f);
    
    std::cout << "180 degrees = " << radians << " radians" << std::endl;
    std::cout << "PI radians = " << degrees << " degrees" << std::endl;
    std::cout << "Lerp(0, 10, 0.5) = " << lerpValue << std::endl;
